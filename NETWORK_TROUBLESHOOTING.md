...
Updated `app.json` to allow local network connections:
...
**After updating `app.json`, you MUST rebuild the app:**
...
**Note:** `expo start` alone will NOT apply these changes. You must rebuild the native app.

## Verification Steps

1. **Rebuild the app** (see above)

2. **Verify HereSphere is running**
...
Enable detailed logging by watching Metro bundler output:
...
### 2. "Network request failed" (Old HTTP error)
- **Cause**: Old HTTP code still in use
- **Solution**: Ensure you're using the latest TCP implementation
- **Verify**: Check `HereSphereService.ts` imports `react-native-tcp-socket`

### 3. "Socket error: Connection refused"
...
### 4. Connection succeeds but no data received
- **Cause**: Protocol mismatch or HereSphere not sending data
- **Solutions**:
  - Load a video in HereSphere to trigger status messages
  - Check console logs for "Received keep-alive" messages
  - Verify JSON parsing isn't failing

### 5. "App has been rebuilt after config changes"
- **Cause**: `app.json` changes require native rebuild
- **Solution**: Run `npx expo run:android` or `npx expo run:ios`
- **Note**: Hot reload won't apply these changes
...
### Packet Capture
Use Wireshark on the HereSphere device:
1. Capture on network interface
2. Filter: `tcp.port == 23554`
3. Look for connection attempts from phone
4. Verify TCP handshake completes

### HereSphere Logs
Check HereSphere application logs for:
...
4. Watch Network tab for socket connections
...
def parse_reply(reply: dict):
    return MediaStatusReport(
        timestamp=time.time(),
        connectionState=CONNECTED_AND_PLAYING if reply['playerState'] == 0 else CONNECTED_AND_PAUSED,
...
- `CONNECTED_BUT_NO_FILE`: Connected, keep-alive only
- `CONNECTED_AND_PAUSED`: Video loaded, playerState != 0
- `CONNECTED_AND_PLAYING`: Video playing, playerState == 0
